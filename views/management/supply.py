...
def render(scope: pd.DataFrame, ctx: Ctx) -> None:
    ...
    st.info(ctx.tf(UNFILTERED_NOTE, what=ctx.t("Supply & cost")), icon=":material/filter_alt_off:")
    if erp.db_present(erp.CALC_DB):
        ...
        db_notice(erp.CALC_DB, ctx.t("Component price inflation"), ctx)
        st.divider()
    _supplier_spend(ctx)
    st.divider()
...
def _component_inflation(ctx: Ctx) -> None:
    st.subheader(ctx.t("Component price inflation"))
    ...
    if ch.empty:
        st.caption(ctx.t("No component price changes in range."))
        return
    P = ctx.P
    ...
    c1.metric(ctx.t("Parts repriced up (last 12m)"), f"{len(ups):,}", border=True,
              help=f"Average increase +{ups['pct_change'].mean():.0f}%." if len(ups) else None)
    c2.metric(ctx.t("Parts repriced down (last 12m)"), f"{len(downs):,}", border=True,
              help=f"Average decrease {downs['pct_change'].mean():.0f}%." if len(downs) else None)
    net = last12["pct_change"].mean()
    c3.metric(ctx.t("Net avg change (last 12m)"), f"{net:+.1f}%", border=True,
              help=ctx.t("Mean % change across all recorded part reprices in the last 12 months. "
    ...
    for col in ("up", "down", "flat"):
        if col not in m:
            m[col] = 0
    fig = go.Figure()
    fig.add_bar(x=m["month"], y=m["up"], name=ctx.t("Increases"),
                marker=dict(color=P["neg"], line=dict(width=0)),
                hovertemplate="<b>%{x|%b %Y}</b><br>%{y} increases<extra></extra>")
    fig.add_bar(x=m["month"], y=-m["down"], name=ctx.t("Decreases"),
    ...
    fig.update_yaxes(title_text=ctx.t("Parts repriced"))
    st.plotly_chart(fig, width="stretch", theme=None)

    st.markdown(ctx.t("**By buying currency**"))
    ...
    fig2.update_yaxes(title_text=ctx.t("Mean % change"))
    st.plotly_chart(fig2, width="stretch", theme=None)
    st.caption(ctx.tf("{n} recorded part reprices {lo}–{hi} (`eurocycles_db_calc.hisprix`). "
                      "Currencies with ≥ 20 changes.",
                      n=f"{len(ch):,}", lo=ctx.yr_lo, hi=ctx.yr_hi))


def _biggest_moves(ctx: Ctx) -> None:
    st.subheader(ctx.t("Biggest recent part price moves"))
    ch = erp.load_component_price_changes(start_year=min(ctx.yr_lo, 2019))
    ch = ch[ch["pct_change"].between(-95, 500)]
    ch = ch[ch["changed"] >= ch["changed"].max() - pd.Timedelta(days=365)]
    if ch.empty:
        st.caption(ctx.t("No changes in the last 12 months."))
        return
    ...
    fig.update_xaxes(title_text=ctx.t("Price change (%)"))
...
def _supplier_spend(ctx: Ctx) -> None:
    st.subheader(ctx.t("Supplier spend (landed component cost)"))
    si = erp.load_supplier_invoices(start_year=min(ctx.yr_lo, 2022))
    si = si[(si["yr"] >= ctx.yr_lo) & (si["yr"] <= ctx.yr_hi)]
    if si.empty:
        st.caption(ctx.t("No supplier invoices in range."))
        return
    P = ctx.P
    by_yr = si.groupby("yr").agg(goods=("goods_dt", "sum"), freight=("freight_dt", "sum")).reset_index()
    left, right = st.columns(2)
    with left:
        st.markdown(ctx.t("**Spend by year**"))
        fig = go.Figure()
        fig.add_bar(x=by_yr["yr"], y=by_yr["goods"] / ctx.rate, name=ctx.t("Goods"),
                    marker=dict(color=P["series"][0], line=dict(width=0)),
                    hovertemplate="<b>%{x}</b><br>goods %{y:,.0f}<extra></extra>")
        fig.add_bar(x=by_yr["yr"], y=by_yr["freight"] / ctx.rate, name=ctx.t("Freight / customs / insurance"),
        ...
        st.plotly_chart(fig, width="stretch", theme=None)
    with right:
        st.markdown(ctx.t("**Top suppliers**"))
        ...
        st.plotly_chart(fig2, width="stretch", theme=None)
    st.caption(ctx.t("From `facturef` supplier invoices, converted to DT at each invoice's own rate. "
               "Freight bucket = `tim + transit + femb + fdae + fsa + ass + tax`."))


# ------------------------------------------------------ supplier claims ---
def _supplier_claims(ctx: Ctx) -> None:
    st.subheader(ctx.t("Supplier quality / quantity claims"))
    sd = erp.load_supplier_disputes()
    if sd.empty:
        st.caption(ctx.t("No `SDisputes` rows."))
        return
    sd = sd.dropna(subset=["dat"])
    sd["dat"] = pd.to_datetime(sd["dat"], errors="coerce")
    sd = sd[(sd["dat"].dt.year >= ctx.yr_lo) & (sd["dat"].dt.year <= ctx.yr_hi)]
    if sd.empty:
        st.caption(ctx.t("No supplier claims in range (data starts 2024)."))
        return
    P = ctx.P
    ...
    fig.update_xaxes(title_text=ctx.t("Distinct claims"))
    fig.update_layout(margin=dict(l=4, r=54, t=4, b=4), bargap=0.3)
    st.plotly_chart(fig, width="stretch", theme=None)
    st.caption(ctx.tf("{n} claims covering {units} units flagged {lo}–{hi} (`SDisputes` ⋈ "
                      "`SDisputesLine`, data starts 2024). Ranked by claim count; claimed "
                      "value is mixed-currency at line level so not totalled.",
                      n=f"{sd['Id'].nunique():,}", units=f"{sd['qty'].sum():,.0f}",
                      lo=ctx.yr_lo, hi=ctx.yr_hi))


# ------------------------------------------------------------ pipeline ---
def _pipeline(ctx: Ctx) -> None:
    st.subheader(ctx.t("Inbound PO pipeline"))
    po = erp.load_open_pos()
    if po.empty:
        st.caption(ctx.t("No purchase-order lines with a future expected-arrival date."))
        return
    P = ctx.P
    ...
    fig.update_yaxes(tickformat="~s", title_text=ctx.tf("Order value ({ccy})", ccy=ctx.ccy))
    st.plotly_chart(fig, width="stretch", theme=None)
    st.caption(ctx.tf("{n} PO lines · {value} expected ahead, by `commandf_det.eta`. "
                      "`fermeecmdf` (closed flag) is unpopulated in this copy, so a forward "
                      "ETA is the 'still awaited' proxy.",
                      n=f"{len(po):,}",
                      value=f"{ctx.ccy} {po['line_value_dt'].sum() / ctx.rate / 1e6:.1f}M"))
