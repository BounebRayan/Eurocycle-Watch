"""Distributor view — the original Apollo / Halfords watch.

Daily snapshots of Apollo-branded bikes as listed on Halfords.com: price and
discount drift, catalog membership, and — best-effort, when the enrich pass
gets past Akamai — stock, rating and merch badges. Reads the committed SQLite
snapshot (`data/apollo_dashboard.db`), unchanged from the single-file version.

This is one distributor's *outward* market picture. The internal cost/margin/
production picture for every distributor lives in the Management view.
"""
...
from theme import FONT, palette, style_fig, hbar_categories, money_gbp as money

DB_PATH = Path(__file__).resolve().parent.parent / "data" / "apollo_dashboard.db"
...
def render() -> None:
    ...
    latest, enrich_stamps = carry_forward(
        filtered[filtered["snapshot_date"] == latest_date], latest_date)
    prev = None
